...
enum { SchemeNorm, SchemeSel, SchemeNormHighlight, SchemeSelHighlight,
       SchemeOut, SchemeSelOut, SchemeLast }; /* color schemes */
...
keypress(XKeyEvent *ev)
{
	...
	if (ev->state & ControlMask) {
		...
		case XK_k: navhistory(-1); buf[0]=0; break;
		case XK_j: navhistory(1); buf[0]=0; break;
...
usage(void)
{
	fputs("usage: dmenu [-bfivP] [-l lines] [-p prompt] [-fn font] [-m monitor] [-H histfile]\n"
	      "             [-h height] [-x xoffset] [-y yoffset] [-w width] [-nhb color] [-nhf color]\n"
